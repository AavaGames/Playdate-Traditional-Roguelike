function ComputeShadow(origin, world, visionRange, is_blocking, mark_visible)
    ...
    for i = 1, 4, 1 do
        ...
        local function scan(row)
            ...
            for index, tile in ipairs(tiles) do
                ...
                local distance = distance(tile[1], tile[2])
                if (distance > visionRange) then
                    ...
                    if is_wall(prev_tile) and is_floor(tile) then
                        row.start_slope = slope_simple(tile)
                    end
                    if is_floor(prev_tile) and is_wall(tile) then
                        local next_row = row:next()
                        next_row.end_slope = slope_simple(tile)
                        scan(next_row)
                    end
                    prev_tile = tile
                end
